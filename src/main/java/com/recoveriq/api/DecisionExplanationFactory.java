...
public final class DecisionExplanationFactory {
    public record TraceItem(String action,double successProbability,double expectedRecoveryValue,double actionCost,double frictionPenalty,boolean selected,String status){ }
    ...
    public static ExplainedDecision explain(FailureType failure,NextBestActionResult result){
        var trace=result.evaluatedActions().stream().map(e->new TraceItem(e.action().name(),e.predictedSuccessProbability(),e.expectedRecoveryValue(),e.actionCost(),e.frictionPenalty(),e.eligible()&&result.selectedAction().filter(a->a==e.action()).isPresent(),e.reason())).toList();
